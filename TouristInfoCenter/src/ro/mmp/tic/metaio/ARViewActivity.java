...
public abstract class ARViewActivity extends Activity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			if (!nativeLibsLoaded)
				throw new Exception(
						"Unsupported platform, failed to load the native libs");
			...
			metaioSDK = MetaioSDK.CreateMetaioSDKAndroid(this, getResources()
					.getString(R.string.mySdkSignature));
			...
			if (layout != 0) {
				...
				if (mGUIView == null)
					MetaioDebug.log(Log.ERROR,
							"ARViewActivity: error inflating the given layout: "
									+ layout);
			}
		} catch (Exception e) {
			MetaioDebug.log(Log.ERROR,
					"ARViewActivity.onCreate: failed to create or intialize metaio SDK: "
							+ e.getMessage());
			finish();
		}
	...
	protected void onResume() {
		...
		if (mSurfaceView != null) {
			// Add rendering surface view here to ensure it is placed on top of
			// the camera view. Using
			// bringToFront() will not work because SurfaceViews seem to disobey
			// the View order within
			// their parent, but instead the order of adding them is
			// significant.
			if (mSurfaceView.getParent() == null) {
				MetaioDebug
						.log("ARViewActivity.onResume: addContentView(mSurfaceView)");
	...
	public void onSurfaceCreated() {
		MetaioDebug.log("ARViewActivity.onSurfaceCreated: GL thread: "
		...
		try {
			...
			if (!mRendererInitialized) {
				...
				// Add loadContent to the event queue to allow rendering to
				// start
				...
				MetaioDebug
						.log("ARViewActivity.onSurfaceCreated: Reloading textures...");
				metaioSDK.reloadTextures();
			}

			// connect the audio callbacks
			MetaioDebug
					.log("ARViewActivity.onSurfaceCreated: Registering audio renderer...");
			...
			MetaioDebug.log(Log.ERROR,
					"ARViewActivity.onSurfaceCreated: " + e.getMessage());
		}
	}
	...
	public void onSurfaceChanged(int width, int height) {
		MetaioDebug.log("ARViewActivity.onSurfaceChanged: " + width + ", "
				+ height);
